...
sketch_create(int64 total_elems, int sketch_work_mem, uint64 seed, int bits_per_elem)
{
	...
	sketch = palloc0(offsetof(frequency_sketch, bitset) + bitset_bytes);
	...
	sketch->reset_counter = 0;
	sketch->size = bitset_bits / BLOCK_SIZE;
...
uint64
sketch_used_mem(frequency_sketch *sketch)
{
	return sketch->size * BLOCK_SIZE / BITS_PER_BYTE;
}
...
sketch_add_element(frequency_sketch *sketch, unsigned char *elem, size_t len)
{
	...
	if (++sketch->reset_counter == sketch->size * BLOCK_SIZE / sketch->bits_per_elem)
	{
		sketch_reset(sketch);
	}
}
...
static void
k_hashes(frequency_sketch *sketch, uint32 *hashes, unsigned char *elem, size_t len)
{
	uint64		hash;
	uint32		x;
	uint32      y;
	...
	hash = DatumGetUInt64(hash_any_extended(elem, len, sketch->seed));
	x = (uint32) hash;
	y = (uint32) (hash >> 32);
	m = sketch->size;
	...
	/* Accumulate hashes */
	hashes[0] = x * sketch->bits_per_elem;
	for (i = 1; i < sketch->k_hash_funcs; i++)
	{
		x = mod_m(x + y, m);
		y = mod_m(y + i, m);

		hashes[i] = x * sketch->bits_per_elem;
	}
}
